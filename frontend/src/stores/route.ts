...
export const useRouteStore = defineStore('route', () => {
    ...
    const submitWaypoint = async (waypoint: { latitude: number; longitude: number; text_note?: string }) => {
    ...
    const submitObservation = async (observation: { latitude: number; longitude: number; type: string; content: string; image?: string; audio?: string }) => {
    ...
    const finalizeRoute = async () => {
        ...
        try {
            await apiClient.post('/route/finalize', {
                route_id: currentRoute.value.id,
                end_time: new Date().toISOString()
            })
        } catch (error) {
            console.warn('Failed to finalize route, mocking success:', error)
        }
